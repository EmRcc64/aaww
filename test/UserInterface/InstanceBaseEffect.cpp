...
void CInstanceBase::__AttachEmpireEffect(DWORD eEmpire)
{
	...
	if (IsBuffNPC())
		return;
#endif
#ifdef ENABLE_NO_KINGDOM_FLAG_ABOVE_PREMIUM
	if (IsPremium())
		return;
#endif
...
void CInstanceBase::UpdateTextTailLevel(DWORD level)
{
	...
	char szText[256];
//#if (defined ENABLE_PREMIUM_SYSTEM && defined ENABLE_PREMIUM_PREFIX)
//	if (IsPremium())
//		sprintf(szText, "Lv %d |cffFFC125[VIP]|r", level);
//	else
...
{
	g_TitlePrestigeNameMap.insert(std::make_pair(iIndex, c_szTitlePrestigeName));
}
#endif
...
void CInstanceBase::SetMetinQueueEffect(const bool bStatus)
{
	if (bStatus)
