...
namespace quest
{
...
#ifdef ENABLE_CHANNEL_SWITCHER //If it does not compile, use "int pc_channel_switch(luaState* L)"
	ALUA(pc_channel_switch)
	{
		if(!lua_isnumber(L, 1))
			return 1;
		
		int new_ch = lua_tonumber(L, 1);
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		ch->ChatPacket(CHAT_TYPE_COMMAND, "channel_switch_command %d", new_ch);
		return 1;
	}
#endif
	...
	ALUA(pc_select_vid)
	{
		...
		if (new_ch)
		{
			...
			lua_pushnumber(L, ch ? (DWORD)ch->GetVID() : 0); //@fixme1012
		}
		else
		{
			lua_pushnumber(L, 0);
...
teleport_area:
	...
	ALUA(pc_give_award_socket)
	{
		...
		if (!lua_isnumber(L, 1) || !lua_isnumber(L, 2) || !lua_isstring(L, 3) || !lua_isstring(L, 4) || !lua_isstring(L, 5) || !lua_isstring(L, 6) 
			...
			|| !lua_isstring(L, 7) || !lua_isstring(L, 8) || !lua_isstring(L, 9) 
#endif
		)		{
			sys_err("QUEST give award call error : wrong argument");
			lua_pushnumber (L, 0);
	...
	ALUA(pc_sf_stun)
	{
		...
		if (lua_toboolean(L, 1) && lua_tonumber(L, 2))
		{
			ch->AddAffect(AFFECT_STUN, 0, 0, AFF_STUN, (int)lua_tonumber(L, 2), 0, 1, 0);
			return 0;
		}
#endif
		if(lua_toboolean(L, 1))
	...
	int pc_calculate_gaya_piece(lua_State* L)
	{
		...
		if (ch)
		{
			...
			for (WORD j = 0; j < INVENTORY_AND_EQUIP_SLOT_MAX; ++j)
			{
				...
				if (item->IsStone())
				{
					if (item->GetRefineLevel() >= 0 && item->GetRefineLevel() <= 4)
			...
			if (findCount)
			{
				...
				for (WORD j = 0; j < INVENTORY_AND_EQUIP_SLOT_MAX; ++j)
				{
					...
					if (item->IsStone())
					{
						if (item->GetRefineLevel() >= 0 && item->GetRefineLevel() <= 4)
						{
							const DWORD removeCount = item->GetCount() > findCount ? findCount : item->GetCount();
...
#ifdef ENABLE_PREMIUM_SYSTEM
	//SYSTEM FUNCTIONS OF PREMIUM SYSTEM
	ALUA(pc_refresh_premium)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	ALUA(pc_premium_expiration_check)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		CPremiumSystem::instance().ExpirationCheck(ch);
		return 1;
	}
	
	ALUA(pc_premium_box_check)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		CPremiumSystem::instance().BoxCheck(ch);
		return 1;
	}
	
	//PLAYER FUNCTIONS OF PREMIUM SYSTEM
	ALUA(pc_is_premium)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (CPremiumSystem::instance().IsPremium(ch))
			lua_pushboolean(L, true);
		else
			lua_pushboolean(L, false);
		return 1;
	...
	ALUA(pc_set_premium)
	{
		...
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		CPremiumSystem::instance().SetPremium(ch, days);
		return 1;
	}
	
	ALUA(pc_remove_premium)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		CPremiumSystem::instance().RemovePremium(ch);
		return 1;
	}
#endif
...
# endif
	...
	ALUA(pc_get_conqueror_next_exp)
	{
		LPCHARACTER pkChar = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	void RegisterPCFunctionTable()
	{
		luaL_reg pc_functions[] =
		{
			...
			{ "channel_switch",			pc_channel_switch	},
#endif
...
#ifdef ENABLE_PREMIUM_SYSTEM
			//SYSTEM FUNCTIONS OF PREMIUM SYSTEM
			{ "refresh_premium",		pc_refresh_premium		},
			{ "premium_expiration_check",		pc_premium_expiration_check		},
			{ "premium_box_check",		pc_premium_box_check	},
			
			//PLAYER FUNCTIONS OF PREMIUM SYSTEM
			{ "is_premium",				pc_is_premium	},
			{ "is_premium2",			pc_is_premium2	},
			{ "set_premium",			pc_set_premium	},
			{ "remove_premium",			pc_remove_premium	},
#endif
			...
			{ "is_observer_mode",			pc_is_observer_mode },

#ifdef ENABLE_DEATHMATCH_SYSTEM
			{ "set_pkmode",			pc_set_pkmode },
#endif
