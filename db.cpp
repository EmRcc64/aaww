...
//	CLoginSim* ld = new CLoginSim;
    auto ld = std::make_unique<CLoginSim>();
//	std::unique_ptr<CLoginSim> ld(new CLoginSim);
...
//    	CLoginQueue::instance().AddNewClient(ld.get());
//		CLoginQueue::instance().AddNewClient(std::move(ld));
	if (ld)
    		CLoginQueue::instance().AddNewClient(ld.release());
...
void DBManager::AnalyzeReturnQuery(SQLMsg * pMsg)
{
    ...
    szClientIp[sizeof(szClientIp)-1] = '\0';
    DESC_MANAGER::instance().LoginSuccessByMacAddr(szClientIp);


//						DESC_MANAGER::instance().LoginSuccessByMacAddr(pinfo->hwid);
#endif
							char szQuery[1024];
#ifdef ENABLE_HWID_BAN
...
#ifdef __MULTI_LANGUAGE_SYSTEM__
							snprintf(szQuery, sizeof(szQuery), "UPDATE account SET last_play=NOW(), language=%u , hwid='%s' WHERE id=%u", pinfo->bLanguage, szHWID_new2, dwID);
#else
...
case QID_CHECK_PROMO:
{
    ...
    const char* code = (const char*)qi->pvData;

    sys_log(0, "Player %s [%d] tries to use promo code %s", ch->GetName(), pid, code);
    ...
    MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
    // Only one promo code per query now!
    int promo_id, item_vnum, item_count, receiver;
    char promo_code[33];
    int col = 0;

    str_to_number(promo_id, row[col++]);
    strlcpy(promo_code, row[col++], sizeof(promo_code));
    str_to_number(item_vnum, row[col++]);
    str_to_number(item_count, row[col++]);
    str_to_number(receiver, row[col++]);

    if (strcmp(promo_code, code) != 0)
    {
        ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("promo code doesn't exist"));
        break;
    }
    ...
    save->item_vnum = item_vnum;
    save->item_count = item_count;

    ReturnQuery(QID_SAVE_PROMO, pid, save,
        "UPDATE `promo_table` SET `receiver` = %d WHERE `promo_id` = %d AND code = '%s' AND receiver = 0",
        pid, promo_id, promo_code);
}
    break;
...
case QID_SAVE_PROMO:
{
    ...
    {
        sys_err("failed to save data in database, won't give an item");
        ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("promo code was already claimed"));
    }
}
    break;
#endif
