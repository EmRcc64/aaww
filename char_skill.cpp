...
static const DWORD s_adwSubSkillVnums[] =
{
	...
	SKILL_PASSIVE_RHH,
	SKILL_PASSIVE_HH,
	SKILL_PASSIVE_RESIS_CRIT,
	SKILL_PASSIVE_RESIS_PERF,
	SKILL_PASSIVE_DROP,
#endif
...
{
	for (BYTE skillIndex = SKILL_PASSIVE_MONSTERS; skillIndex <= SKILL_PASSIVE_RESIS_PERF; ++skillIndex)
...
bool CHARACTER::LearnSkillByBook(DWORD dwSkillVnum, BYTE bProb)
{
	...
	else
	{
...

		{
			...
			PointChange(POINT_EXP, static_cast<int>(need_exp)*(-1));
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
		...
		char szBufSkill[256];
		snprintf(szBufSkill, sizeof(szBufSkill), "damage_skill_define_%u", m_pkSk->dwVnum);
		if(m_pkChr->IsInTacticMap() && quest::CQuestManager::instance().GetEventFlag(szBufSkill) > 0)
			iDam = quest::CQuestManager::instance().GetEventFlag(szBufSkill);
#endif	

		if (!pkChrVictim->Damage(m_pkChr, iDam, dt) && !pkChrVictim->IsStun() && !pkChrVictim->IsDead() /*@fixme1076*/)
		{
...
#ifdef ENABLE_MAP_PVP_TACT
			if (!(m_pkChr->IsPC() && m_pkChr->IsInTacticMap() && pkChrVictim->IsPC()))
#endif
			{
#	ifdef ENABLE_WOLFMAN_CHARACTER
...
void CHARACTER::SkillLearnWaitMoreTimeMessage(DWORD ms)
{
	...
	if (ms < 3 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("Im burning inside, but it soothes my body. My chi needs to stabilize."));
	else if (ms < 5 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("Kinda slow...but continously...non-stop!"));
	else if (ms < 10 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("It feels good. My body is full of Chi."));
	else if (ms < 30 * 60)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("I read it! Now the Chi has to spread in my body."));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("Training its over!"));
	}
	else if (ms < 1 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("I have reched the last page of the book Training its almost over!"));
	else if (ms < 2 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("I almost made it! A little more!"));
	else if (ms < 3 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("Im almost done reading!"));
	else if (ms < 6 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("Only a few pages left,then ill finish reading."));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("I feel refreshed."));
	}
	else if (ms < 12 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("Now i get it!."));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("Okay, I need to stay focused!"));
	}
	else if (ms < 18 * 3600)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("I keep reading the same line."));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("I dont want to study further."));
	}
	else //if (ms < 2 * 86400)
	{
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("Its more complicated and harder to understand than i tought."));
		ChatPacket(CHAT_TYPE_TALKING, "%s", LC_TEXT("Its hard to concentrate. I should take a break."));
	}
	/*
	   str = "30%";
	   else if (ms < 3 * 86400)
