...
bool DESC_MANAGER::LoginCheckByMacAddr(const std::string& c_rstMacAddr, const std::string& c_rstLogin, const std::string& c_rstPassword)
{
	TAntiBruteforceInfo* pkInfo;

	itertype(m_map_AntiBruteforceByMacAddr) it = m_map_AntiBruteforceByMacAddr.find(c_rstMacAddr);
	if (it == m_map_AntiBruteforceByMacAddr.end())
	{
		pkInfo = &m_map_AntiBruteforceByMacAddr[c_rstMacAddr];
	...
	if (pkInfo->bCount >= ANTI_LOGIN_BRUTEFORCE_MAX_TRY){
		std::unique_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery("INSERT INTO log.log_bruteforce (login_entered, password_entered, login_time, mac_addr) VALUES ('%s', '%s', NOW(), '%s')", c_rstLogin.c_str(), c_rstPassword.c_str(), c_rstMacAddr.c_str()));
		return false;
	}
...
void DESC_MANAGER::LoginSuccessByMacAddr(const std::string& c_rstMacAddr)
{
	itertype(m_map_AntiBruteforceByMacAddr) it = m_map_AntiBruteforceByMacAddr.find(c_rstMacAddr);
	if (it == m_map_AntiBruteforceByMacAddr.end())
		return;
