...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	snprintf(msg, sizeof(msg), LC_TEXT("%s challenged you to a duel!"), pkChr->GetName());
...
bool CPVPManager::CanAttack(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if (pkVictim->IsBuffNPC())
		return false;
#endif

#ifdef PET_ATTACK
    if (pkChr->IsPet() && pkVictim->IsPC())
    {
        return false;
    }
    if (pkChr->IsPet() && pkVictim->IsMonster())
    {
     ...
     if (pkChr->IsPet() && pkVictim->IsStone())
    {
        return true;
    }
    if (pkChr->IsMonster() && pkVictim->IsPet()) { return true; }
#endif
...
{
	// Allow damage regardless of horse level and grade
	// Remove the existing check for horse level and grade
}
else
{
	// Remove the mount type restrictions
	// Comment out or remove the switch-case block
	/*
	...
	*/
...
#ifdef QUEST_PROTECT_EMPIRE
	int protect_empire_state = quest::CQuestManager::instance().GetEventFlag("protectempire_event");
	int shinsoo_stone = PROTECT_EMPIRE_SHINSOO_STONE;
	int chunjo_stone = PROTECT_EMPIRE_CHUNJO_STONE;
	int jinno_stone = PROTECT_EMPIRE_JINNO_STONE;
	int shinsoo_badge = PROTECT_EMPIRE_SHINSOO_BADGE;
	int chunjo_badge = PROTECT_EMPIRE_CHUNJO_BADGE;
	int jinno_badge = PROTECT_EMPIRE_JINNO_BADGE;
	if (pkVictim->GetMapIndex() == PROTECT_EMPIRE_MAP_INDEX)
	{
		if (protect_empire_state == 1)
			return false;
		else if (protect_empire_state == 2)
		{
			if (pkVictim->IsPC() && pkVictim->GetEmpire() == pkChr->GetEmpire())
				return false;
			if (pkVictim->GetRaceNum() == shinsoo_stone && pkChr->GetEmpire() == 1)
				return false;
			else if (pkVictim->GetRaceNum() == chunjo_stone && pkChr->GetEmpire() == 2)
				return false;
			else if (pkVictim->GetRaceNum() == jinno_stone && pkChr->GetEmpire() == 3)
				return false;
				
			if (pkVictim->GetRaceNum() == shinsoo_stone && !pkChr->CountSpecifyItem(shinsoo_badge))
				return false;
			else if (pkVictim->GetRaceNum() == chunjo_stone && !pkChr->CountSpecifyItem(chunjo_badge))
				return false;
			else if (pkVictim->GetRaceNum() == jinno_stone && !pkChr->CountSpecifyItem(jinno_badge))
				return false;
		}
	}
#endif
	...
	if (pkChr->GetEmpire() != pkVictim->GetEmpire()
		...
		&& pkChr->GetMapIndex() != DEATHMATCH_MAP_INDEX
#endif
	)
	{
		// @warme005
