...
namespace quest
{
	...
	ALUA(item_bk_select_cell)
	{
		...
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		LPITEM item = ch ? ch->GetBookInventoryItem(cell) : NULL;
...
#define NS_ITEM_GETMODE0(x)	\
	...
	ALUA(item_set_count0)
	{
		...
		if (item)
			...
			//item->SetCount(MINMAX(1, lua_tonumber(L, 1), g_wItemCountLimit));
	...
	void RegisterITEMFunctionTable()
	{
		luaL_reg item_functions[] =
		{
			...
			#ifdef ENABLE_SPECIAL_STORAGE
			{ "select_bk_cell",	item_bk_select_cell	},
			#endif
			{ "remove",		item_remove		},
