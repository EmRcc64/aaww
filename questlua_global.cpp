...
namespace quest
{
	...
	ALUA(_locale_quest)
	{
		if (lua_isnumber(L, 1))
		{
			...
			if (ch)
			{
				...
				char szBuf[1024 + 1] = {};
				snprintf(szBuf, sizeof(szBuf), "%s ", LC_QUEST(lua_tonumber(L, 1), bLocale));
	...
	ALUA(_item_name)
	{
		if (lua_isnumber(L,1))
		{
...
#ifdef __MULTI_LANGUAGE_SYSTEM__
			{
				...
				lua_pushstring(L, LC_ITEM_NAME(dwVnum, bLocale));
			}
#else
	...
	ALUA(_mob_name)
	{
		if (lua_isnumber(L, 1))
		{
...
#ifdef __MULTI_LANGUAGE_SYSTEM__
			{
				LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
				BYTE bLocale = (ch ? ch->GetDesc()->GetLanguage() : LOCALE_DEFAULT);
				lua_pushstring(L, LC_MOB_NAME(dwVnum, bLocale));
			}
#else
	...
	ALUA(_spawn_mob0)
	{
		...
		const DWORD iRot = static_cast<DWORD>(lua_tonumber(L, 5));
		...
		const LPCHARACTER ch = CHARACTER_MANAGER::instance().SpawnMob(dwVnum, lMapIndex, pkSectreeMap->m_setting.iBaseX+dwX*100, pkSectreeMap->m_setting.iBaseY+dwY*100, 0, false, iRot?iRot:-1);
...
#define NAN (INFINITY-INFINITY)
	...
	struct FStunCharacters
	{
		bool stun_state;
		
		FStunCharacters(bool st)
			: stun_state(st)
		{}
		...
		void operator() (LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC() && !ch->IsGM())
				{
					if (ch->FindAffect(AFFECT_STUN))
						ch->RemoveAffect(AFFECT_STUN);
					if (stun_state)
						ch->AddAffect(AFFECT_STUN, 0, 0, AFF_STUN, 60*60*60*365, 0, 1, 0);
				}
			}
		}
	};

	ALUA(_set_stun_in_map)
	{
		bool arg = true;
		...
		if (pChar)
		{
			LPSECTREE_MAP sectree = SECTREE_MANAGER::instance().GetMap(pChar->GetMapIndex());
		
			if (sectree != NULL)
			{
				struct FStunCharacters f(arg);
				sectree->for_each( f );
			}
		}
		return 0;
...
#ifdef ENABLE_DEATHMATCH_SYSTEM
			{	"set_stun_in_map", 				_set_stun_in_map					},
#endif
