...
class CPythonExchange : public CSingleton<CPythonExchange>
{
	public:
		...
		typedef struct trade
		{
			...
			uint16_t					item_count[EXCHANGE_ITEM_MAX_NUM];
	...
	public:
		...
		void			SetItemToTarget(DWORD pos, DWORD vnum, uint16_t count);
		void			SetItemToSelf(DWORD pos, DWORD vnum, uint16_t count);
		...
		uint16_t			GetItemCountFromTarget(BYTE pos);
		uint16_t			GetItemCountFromSelf(BYTE pos);
