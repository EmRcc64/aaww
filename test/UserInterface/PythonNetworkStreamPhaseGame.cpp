...
void CPythonNetworkStream::GamePhase()
{
    ...
    while (ret)
	{
		...
		case HEADER_GC_EVENT_MANAGER:
			...
			case HEADER_GC_REFRESH_PREMIUM:
				ret = RecvRefreshPremiumPacket();
				break;
#endif
...
bool CPythonNetworkStream::RecvMaintenance()
{
	...
	if (!Recv(sizeof(TPacketGCMaintenance), &p))
		return false;
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetMaintenance", Py_BuildValue("(iiiis)", p.maintenance.mode, p.maintenance.iStartTime, p.maintenance.iEndTime, p.maintenance.status, p.maintenance.reason));
	return true;
}
#endif
	...
	switch (exchange_packet.subheader)
	{
		...
		case EXCHANGE_SUBHEADER_GC_ITEM_ADD:
			if (exchange_packet.is_me)
			{
				...
				CPythonExchange::Instance().SetItemToSelf(iSlotIndex, exchange_packet.arg1, (uint16_t) exchange_packet.arg3);
			...
			{
				int iSlotIndex = exchange_packet.arg2.cell;
				CPythonExchange::Instance().SetItemToTarget(iSlotIndex, exchange_packet.arg1, (uint16_t) exchange_packet.arg3);
				for (int i = 0; i < ITEM_SOCKET_SLOT_MAX_NUM; ++i)
	...
	if (pInstPlayer && pInstTarget)
	{
		if (!pInstTarget->IsDead())
		{
			...
			if (pInstTarget->IsPC() || pInstTarget->IsBuilding())
				...
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetHPTargetBoard", Py_BuildValue("(iiLL)", TargetPacket.dwVID, TargetPacket.bHPPercent, TargetPacket.dwMinHP, TargetPacket.dwMaxHP));
#else
			...
			m_pInstTarget = pInstTarget;
				...
				for (BYTE bElementIndex = 0; bElementIndex < CPythonNonPlayer::MOB_ELEMENT_MAX_NUM; ++bElementIndex)
					PyDict_SetItem(poDict, Py_BuildValue("i", bElementIndex), Py_BuildValue("i", TargetPacket.bElement[bElementIndex]));
...
#if defined(ENABLE_WAR_MAP_PVP_TACT)
						Py_BuildValue("(iib)", kGuildWar.dwGuildOpp, kGuildWar.bType, kGuildWar.isTactic)
#else
...
bool CPythonNetworkStream::RecvAffectAddPacket()
{
	...
	if (rkElement.dwType == CInstanceBase::NEW_AFFECT_METIN_QUEUE)
		CPythonPlayer::Instance().SetMetinstonesQueueCount(5);
#endif
...
void CPythonNetworkStream::Discord_Update(const bool ingame)
{
	...
	discordPresence.buttonLabel = "¡Join to Celeste2!";
	discordPresence.buttonURL = "https://discord.gg/aHn3TeNyed";
#endif

//	if (!ingame)
//	{
//
//		auto LogoData = Discord::GetLogoServer();
//		discordPresence.largeImageKey = std::get<0>(LogoData).c_str();
//		discordPresence.largeImageText = std::get<1>(LogoData).c_str();
//
			...
			for (DWORD j = 0; j < categoryTotalSize; ++j)
			{
				...
				for (DWORD x = 0; x < categorySize; ++x)
				{
					...
					if (categorySubSize)
					{
						...
						if (!Recv(sizeof(TIShopData) * categorySubSize, m_vec.data()))
							return false;
		...
		for (BYTE j = 0; j < dayCount; ++j)
		{
			...
			if (dayEventCount > 0)
			{
				...
				for (const auto& eventPtr : m_vec)
				{
					...
					FormatEventTime(eventPtr.startTime, startTimeText, sizeof(startTimeText));
					FormatEventTime(eventPtr.endTime, endTimeText, sizeof(endTimeText));
...
void CPythonNetworkStream::FormatEventTime(time_t timeValue, char* buffer, size_t bufferSize)
{
	const struct tm vEventTime = *localtime(&timeValue);
	snprintf(buffer, bufferSize, "%d-%02d-%02d %02d:%02d:%02d", vEventTime.tm_year + 1900, vEventTime.tm_mon + 1, vEventTime.tm_mday, vEventTime.tm_hour, vEventTime.tm_min, vEventTime.tm_sec);
}
#endif
...
#ifdef ENABLE_PREMIUM_SYSTEM
bool CPythonNetworkStream::RecvRefreshPremiumPacket()
{
	TPacketGCRefreshPremium RefreshPremiumGC;

	if (!Recv(sizeof(TPacketGCRefreshPremium), &RefreshPremiumGC))
	{
		Tracen("Recv RefreshPremiumGC Packet Error");
		return false;
	}

	CInstanceBase * pInstance = CPythonCharacterManager::Instance().GetMainInstancePtr();
	pInstance->UpdateTextTailLevel(pInstance->GetLevel());
	return true;
}
#endif
