	...
	for (BYTE bSlot = 0; bSlot < SWITCHBOT_SLOT_COUNT; ++bSlot)
	{
		...
		if (CheckItem(pkItem, bSlot))
		{
			...
			if (desc)
			{
				...
				strlcpy(pack.szNameFrom, "[Switchboot]", sizeof(pack.szNameFrom));
...
void CSwitchbotManager::Start(DWORD player_id, BYTE slot, std::vector<TSwitchbotAttributeAlternativeTable> vec_alternatives)
{
	...
	if (!pkSwitchbot)
	{
		sys_err("El dopador automático no encontro player_id %d ranura %d", player_id, slot);
		return;
	}

	if (pkSwitchbot->IsActive(slot))
	{
		sys_err("Dopador Ranura %d en este momento esta trabajando player_id %d", slot, player_id);
		return;
	}
...
void CSwitchbotManager::Stop(DWORD player_id, BYTE slot)
{
	...
	if (!pkSwitchbot->IsActive(slot))
	{
		sys_err("Dopador Ranura %d en este momento no esta trabajando player_id %d", slot, player_id);
		return;
	}
