...
class CPythonPlayer : public CSingleton<CPythonPlayer>, public IAbstractPlayer
{
	...
	public:
		...
		void	SetItemCount(TItemPos Cell, uint16_t byCount);
	...
	public:
		...
		bool		SetMetinstonesQueue(const bool isAdd, const DWORD dwVirtualID);
		DWORD		GetMetinstonesQueueTarget();
		void		SetMetinstonesQueueCount(const BYTE bCount) { bQueueCount = bCount; }
		void		SetMetinstonesQueueCount2(const BYTE bCount) {
			if (bQueueCount < bCount)
				bQueueCount = bCount;
		}
