...
#define ANTI_LOGIN_BRUTEFORCE_MAX_TRY 5
#define ANTI_LOGIN_BRUTEFORCE_TIMELIMIT (1000*60*30)
#endif
...
class DESC_MANAGER : public singleton<DESC_MANAGER>
{
...
#ifdef ENABLE_ANTI_LOGIN_BRUTEFORCE
		bool			LoginCheckByMacAddr(const std::string& c_rstMacAddr, const std::string& c_rstLogin, const std::string& c_rstPassword);
		void			LoginSuccessByMacAddr(const std::string& c_rstMacAddr);
#endif
	...
	private:
		...
		std::map<std::string, TAntiBruteforceInfo>	m_map_AntiBruteforceByMacAddr;
#endif
