...
int	CMapOutdoor::__RenderTerrain_RecurseRenderQuadTree_CheckBoundingCircle(const D3DXVECTOR3& c_v3Center, const float& c_fRadius)
{
	...
	for (i = 0; i < count; ++i)
	{
		...
		if (distance[i] <= -c_fRadius * extraRadius)
			return VIEW_NONE;
	}
...
void CMapOutdoor::__RenderTerrain_AppendPatch(const D3DXVECTOR3& c_rv3Center, float fDistance, long lPatchNum)
{
	if (Frustum::Instance().renderShadow != 4)
	{
		if (Frustum::Instance().renderShadow == 3)
		{
...
void CMapOutdoor::RenderArea(bool bRenderAmbience)
{
	...
	if (Frustum::Instance().renderShadow != 4)
	{
		if (Frustum::Instance().isIndoor == false)
		{
			...
			D3DXVECTOR3 sunPosition = current_v3SunPosition;
			sunPosition.z = sunPosition.z / 1.5f;
			STATEMANAGER.SetPixelShaderConstant(1, &sunPosition, 1);
			STATEMANAGER.SetPixelShaderConstant(2, &light.Diffuse, 1);
			STATEMANAGER.SetPixelShaderConstant(3, &Frustum::Instance().AmbientLight, 1);
			// Fog settings
...
void CMapOutdoor::RenderMarkedArea()
{
	...
	STATEMANAGER.SaveTextureStageState(1, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_COUNT2);
#ifdef DIRECTX9
	STATEMANAGER.SaveSamplerState(1, D3DSAMP_MINFILTER, D3DTEXF_ANISOTROPIC);
	STATEMANAGER.SaveSamplerState(1, D3DSAMP_MAGFILTER, D3DTEXF_ANISOTROPIC);
	STATEMANAGER.SaveSamplerState(1, D3DSAMP_MIPFILTER, D3DTEXF_ANISOTROPIC);
...
#else
	STATEMANAGER.SaveTextureStageState(1, D3DTSS_MINFILTER, D3DTEXF_ANISOTROPIC);
	STATEMANAGER.SaveTextureStageState(1, D3DTSS_MAGFILTER, D3DTEXF_ANISOTROPIC);
	STATEMANAGER.SaveTextureStageState(1, D3DTSS_MIPFILTER, D3DTEXF_ANISOTROPIC);
