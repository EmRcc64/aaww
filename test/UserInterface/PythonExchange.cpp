...
void CPythonExchange::SetItemToTarget(DWORD pos, DWORD vnum, uint16_t count)
{
...
void CPythonExchange::SetItemToSelf(DWORD pos, DWORD vnum, uint16_t count)
{
	if (pos >= EXCHANGE_ITEM_MAX_NUM)
		return;
...
uint16_t CPythonExchange::GetItemCountFromTarget(BYTE pos)
{
	if (pos >= EXCHANGE_ITEM_MAX_NUM)
		return 0;
...
uint16_t CPythonExchange::GetItemCountFromSelf(BYTE pos)
{
	if (pos >= EXCHANGE_ITEM_MAX_NUM)
		return 0;
