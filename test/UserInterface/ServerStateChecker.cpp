...
void CServerStateChecker::Update()
{
	...
	if (!m_kStream.Recv(sizeof(TMainteance), &maintenance))
		return;
	PyCallClassMemberFunc(m_poWnd, "SetMaintenance", Py_BuildValue("(iiiis)", maintenance.mode, maintenance.iStartTime, maintenance.iEndTime, maintenance.status, maintenance.reason));
#endif
