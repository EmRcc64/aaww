...
namespace quest
{
	...
	ALUA(mob_spawn)
	{
		...
		BYTE dir = (BYTE) lua_tonumber(L, 7);
		...
		while (count--)
		{
			for (int loop = 0; loop < 8; ++loop)
			{
				...
				if (dir)
					mob = CHARACTER_MANAGER::instance().SpawnMob(mob_vnum, ch->GetMapIndex(), x, y, 0, true, dir);
				else
	...
	ALUA(mob_spawn_map)
	{
		...
		BYTE dir = (BYTE) lua_tonumber(L, 2);
		LPCHARACTER mob = CHARACTER_MANAGER::instance().SpawnMobRandomPosition(vnum, ch->GetMapIndex(), false, dir);
		LPSECTREE_MAP pMap = SECTREE_MANAGER::instance().GetMap(ch->GetMapIndex());
		if (mob)
		{
			if (pMap)
			{
				...
				lua_pushnumber(L, (mob->GetX() - pMap->m_setting.iBaseX) / 100);
				lua_pushnumber(L, (mob->GetY() - pMap->m_setting.iBaseY) / 100);
				return 3;
			}
		}
		else
