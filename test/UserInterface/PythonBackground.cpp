...
bool CPythonBackground::SetShadowTargetLevel(int eLevel)
{
	...
	switch (m_eShadowTargetLevel)
	{
		case SHADOW_NONE:
			...
			Frustum::Instance().renderShadow = 0;
			break;

		case SHADOW_GROUND:
			rkMap.SetDrawShadow(true);
			rkMap.SetDrawCharacterShadow(false);
			Frustum::Instance().renderShadow = 1;
			break;

		case SHADOW_GROUND_AND_SOLO:
			rkMap.SetDrawShadow(true);
			rkMap.SetDrawCharacterShadow(true);
			Frustum::Instance().renderShadow = 2;
			break;

		case SHADOW_ALL:
			rkMap.SetDrawShadow(true);
			rkMap.SetDrawCharacterShadow(true);
			Frustum::Instance().renderShadow = 3;
			break;

		case GRAPHICS_NONE:
			rkMap.SetDrawShadow(true);
			rkMap.SetDrawCharacterShadow(true);
			Frustum::Instance().renderShadow = 4;
			break;
	}
...
void CPythonBackground::SkyUtil()
{
	CMapOutdoor& rkMap = GetMapOutdoorRef();
	rkMap.SkyUtil();
}
...
void CPythonBackground::RenderCharacterShadowToTexture()
{
	...
	CMapOutdoor& rkMap = GetMapOutdoorRef();
	CSpeedTreeForestDirectX8::Instance().Render3(Forest_RenderAll);
}
...
void CPythonBackground::RenderCharacterShadowToTexture3()
{
	...
	if (Frustum::Instance().shadowType != 3)
	{
		...
		if (canRender)
		{
			...
			if (Frustum::Instance().renderShadow == 1)
			{
				rkMap.RenderArea2();
			}

			if (Frustum::Instance().renderShadow == 2 || Frustum::Instance().renderShadow == 3)
			{
			...
			{
				rkMap.__RenderTerrain_RenderHardwareTransformPatch2();
				ms_lpd3dDevice->SetPixelShader(rkMap.m_lpShadowMapPixelShaderDepthTree);
				CSpeedTreeForestDirectX8::Instance().Render2(Forest_RenderAll);	
			}
		}
