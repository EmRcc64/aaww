...
bool CHARACTER::UpdateAffect()
{
	if (GetPoint(POINT_HP_RECOVERY) > 0)
	{
		...
		{
			int64_t iVal = MIN(GetPoint(POINT_HP_RECOVERY), GetMaxHP() * 7 / 100);
	...
	if (GetPoint(POINT_SP_RECOVERY) > 0)
	{
		...
		{
			int64_t iVal = MIN(GetPoint(POINT_SP_RECOVERY), GetMaxSP() * 7 / 100);
...
void CHARACTER::ClearAffect(bool bSave)
{
	...
	while (it != m_list_pkAffect.end())
	{
...
#ifdef METINSTONES_QUEUE
			if (pkAff->dwType == AFFECT_THIEF_GLOVES_STONES)
			{
				++it;
				continue;
			}
...
#ifdef METINSTONES_QUEUE
			if (pkAff->dwType == AFFECT_THIEF_GLOVES_BOSSES)
			{
				++it;
				continue;
			}
		...
		if (pkAff->dwType == AFFECT_SINGLE_BONUS_URIEL)
		{
			++it;
			continue;
		}

		if (pkAff->dwType == AFFECT_SINGLE_BONUS_URIEL_DELAY)
		{
			++it;
			continue;
		}
...
bool CHARACTER::RemoveAffect(CAffect * pkAff, bool single)
{
	...
	if (single) //@fixme1024
	{
		if (AFFECT_REVIVE_INVISIBLE != pkAff->dwType && AFFECT_MOUNT != pkAff->dwType) //@fixme1011
			ComputePoints();
		else  // @fixme110
			UpdatePacket();
	}
...
bool CHARACTER::RemoveAffect(DWORD dwType)
{
	...
	{
		RemoveAffect(pkAff, false); //@fixme1024
		flag = true;
	}
