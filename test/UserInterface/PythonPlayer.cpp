...
DWORD CPythonPlayer::GetLevel()
{
	return GetStatus(POINT_LEVEL);
}
...
void CPythonPlayer::SetStatus(DWORD dwType, int64_t lValue)
{
	...
	if (dwType == POINT_CONQUEROR_LEVEL && lValue > 0)
	{
		CInstanceBase* pkPlayer = NEW_GetMainActorPtr();
		if (pkPlayer)
...
void CPythonPlayer::SetItemCount(TItemPos Cell, uint16_t byCount)
{
	if (!Cell.IsValidCell())
		return;
...
bool CPythonPlayer::LoadNewRegen()
{
	...
	CMappedFile File;
	LPCVOID pData;
	if (!CEterPackManager::Instance().Get(File, "locale/newregen.txt", &pData))
		return false;
