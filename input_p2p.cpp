...
int CInputP2P::PlayerRank(LPDESC d, const char* c_pData, size_t uiBytes)
{
	...
	if (p->lSize == 0)
	{
		...
		RankPlayer::instance().ReceiveP2P(p->iMode, p->dwPid, p->szAccount, p->szName, p->bEmpire, p->bRace, p->lValue);
		return (p->lSize);
	}
	...
	while (cur < iCount)
	{
		...
		RankPlayer::instance().ReceiveP2P(pRank->iMode, pRank->dwPid, pRank->szAccount, pRank->szName, pRank->bEmpire, pRank->bRace, pRank->lValue);
		cur++;
	}
